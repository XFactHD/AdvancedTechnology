...
import xfacthd.advtech.common.menu.machine.ContainerMenuFertilizer;
...
public class BlockEntityFertilizer extends BlockEntityInventoryMachine implements IRangedMachine
{
    public static final Component TITLE = new TranslatableComponent("gui." + AdvancedTechnology.MODID + ".fertilizer");
    ...
    {
        super(ATContent.machineEntity(MachineType.FERTILIZER), pos, state);
    }
    ...
    private void tryFertilizeGrowable()
    {
        ...
        if (state.getBlock() instanceof CropBlock crops)
        {
            if (!crops.isMaxAge(state))
            {
                ...
                boolean success = BoneMealItem.growCrop(stack, level(), scanPos);
                if (success)
                {
                    setChanged();
                }
    ...
    {
        return new ContainerMenuFertilizer(windowId, this, inventory);
    }
