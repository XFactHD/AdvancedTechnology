...
public class BlockCreativeItemSource extends Block implements EntityBlock, IBlockItemProvider
{
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit)
    {
        if (!level.isClientSide())
        {
            ...
            if (level.getBlockEntity(pos) instanceof BlockEntityCreativeItemSource be)
            {
                be.setItem(stack);
            }
        }
    ...
    {
        return new BlockEntityCreativeItemSource(pos, state);
    }
